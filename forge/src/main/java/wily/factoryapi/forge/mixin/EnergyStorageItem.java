    ...
    default ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag nbt) {
        return new ICapabilityProvider() {
            ...
            public @NotNull <T> LazyOptional<T> getCapability(@NotNull Capability<T> capability, @Nullable Direction arg) {
                ...
                if (capability == FactoryCapabilities.CRAFTY_ENERGY && storageItem instanceof ICraftyStorageItem )
                    return LazyOptional.of(()->new SimpleItemCraftyStorage(stack,0,storageItem.getCapacity(),storageItem.getMaxReceive(), storageItem.getMaxConsume(), storageItem.getTransport(), ((ICraftyStorageItem)storageItem).getSupportedEnergyTier() ,stack.getItem() instanceof BlockItem)).cast();
