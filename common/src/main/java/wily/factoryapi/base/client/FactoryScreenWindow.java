...
public abstract class FactoryScreenWindow<T extends AbstractContainerScreen<?>> extends AbstractWidget implements IWindowWidget {
    ...
    public void onClose(){
        ...
        if (parent instanceof IWindowWidget) ((IWindowWidget)parent).getNestedRenderables().forEach(l-> {if (l instanceof FactoryScreenWindow<?>) onClick.accept((FactoryScreenWindow<?>)l);});
    }
    ...
    protected void renderBg(PoseStack stack, int i, int j, float f) {
        ...
        if (useGeneratedBackground) ScreenUtil.drawGUIBackground(stack, getX(), getY(), width, height);
        else drawable.draw(stack,getX(),getY());
