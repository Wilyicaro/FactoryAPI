...
public class ScreenUtil {
    ...
    public static void renderGuiBlock(PoseStack poseStack, @Nullable BlockEntity be, BlockState state, int i, int j, float scaleX, float scaleY, float rotateX, float rotateY) {
        ...
        Consumer<BakedModel> defaultRender = (b)->{
            itemRenderer.render(stack, ItemTransforms.TransformType.NONE, false, poseStack, bufferSource, 15728880, OverlayTexture.NO_OVERLAY,b);
        ...
        if (bakedModel.isCustomRenderer()){
            ...
            if (stack.getItem() instanceof IFactoryItem f) {
                ...
                f.clientExtension(c->{
                    if (c.getCustomRenderer(mc.getBlockEntityRenderDispatcher(), mc.getEntityModels()) instanceof IFactoryBlockEntityWLRenderer r) r.renderByItemBlockState(state, stack, ItemTransforms.TransformType.NONE, poseStack, mc.renderBuffers().bufferSource(),15728880, OverlayTexture.NO_OVERLAY);
                });
