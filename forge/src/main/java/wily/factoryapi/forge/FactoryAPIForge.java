...
public class FactoryAPIForge {
    ...
    public static void attachCapabilities(AttachCapabilitiesEvent<BlockEntity> event){
        if (event.getObject() instanceof IFactoryStorage){
            event.addCapability(new ResourceLocation(FactoryAPI.MOD_ID, "factory_api_capabilities"), new ICapabilityProvider() {
                ...
                public @NotNull <T> LazyOptional<T> getCapability(@NotNull Capability<T> capability, @Nullable Direction arg) {
                    ...
                    ArbitrarySupplier<? extends IPlatformHandlerApi<?>> handler =  ((IFactoryStorage)event.getObject()).getStorage(storage,arg);
