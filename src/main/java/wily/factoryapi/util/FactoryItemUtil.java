...
public class FactoryItemUtil {
    //? if >=1.20.5 {
    public static final Codec<List<Item>> ITEM_COMPONENTS_CODEC = net.minecraft.resources.Identifier.CODEC.xmap(r-> FactoryAPIPlatform.getRegistryValue(r,BuiltInRegistries.ITEM), BuiltInRegistries.ITEM::getKey).listOf();
    ...
    public static List<Item> getItemComponents(ItemStack itemStack){
        ...
        if (itemStack.hasTag() && !itemStack.getTag().getList("Components",8).isEmpty())
            itemStack.getTag().getList("Components",8).forEach(t->list.add(BuiltInRegistries.ITEM.get(new net.minecraft.resources.Identifier(t.getAsString()))));
        return list;
        *///?} else {
